...
현재 사이트는 검색, 내용 성격·출처 필터, 문화 일정 상태, 중복 묶음, 읽음 상태, 출처 즐겨찾기, 긴 자료를 모은 읽기 메뉴, 공개 링크 제보와 공간 지도를 제공합니다. 텔레그램·웹사이트·RSS 같은 전달 경로와 관계없이 기사, 성명·논평, 공식 자료, 행사·일정, 문화·예술, 기록·소식으로 분류합니다. 데이터는 Supabase에서 읽어오며 공개 화면은 로그인 없이 사용할 수 있습니다.
...
- `reading_materials`: 도서, 구술·인터뷰, 토론·포럼, 보고서와 연구자료
...
`읽기` 메뉴는 `reading_materials` 표에서 관리합니다. `status`가 `public`인 항목은 다음 페이지 요청부터 사이트에 표시되며, `featured`를 켜면 목록 앞쪽에 배치됩니다.
