...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const responses = [sourcesResponse, itemsResponse, watchResponse, looseResponse, eventsResponse, placesResponse];
    ...
    const [sources, items, watchlist, loose_ends, events, places] = await Promise.all(responses.map((response) => response.json()));
    ...
    return new Response(JSON.stringify({
      ...
      items, watchlist, loose_ends, events, places,
