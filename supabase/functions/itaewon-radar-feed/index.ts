...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const [sourcesResponse, itemsResponse, watchResponse, looseResponse, eventsResponse] = await Promise.all([
    ...
    const responses = [sourcesResponse, itemsResponse, watchResponse, looseResponse, eventsResponse];
    ...
    const [sources, items, watchlist, loose_ends, events] = await Promise.all(responses.map((response) => response.json()));
    ...
    return new Response(JSON.stringify({
      ...
      items, watchlist, loose_ends, events,
